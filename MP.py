...
def block(win):
    def block_websites(websites):
        ...
        with open(host_file, 'r+') as hostfile:
            ...
            showinfo('Websites blocked!', message='The desired website/s is/are blocked.')
    ...
    Label(blck_wn, text='Block a website', background='LightBlue', font=("Georgia", 16)).place(x=120, y=20)
    Label(blck_wn, text='Enter the URL (www.<sitename>.com):', background='LightBlue', font=('Times', 13)).place(
        x=60, y=90)

    sites = Text(blck_wn, width=35, height=3)
    sites.place(x=60, y=120)
...
def unblock(win):
    def unblock_websites(websites_to_unblock):
        ...
        with open(host_file, 'r+') as hostfile:
            ...
            hostfile.seek(0)
            showinfo('Website Unblocked!', message='The desired website is unblocked now.')
    ...
    with open('blocked_websites.txt', 'r+') as blocked_websites:
        blck_sites = blocked_websites.read().splitlines()[0:]

    unblck_wn = Toplevel(win, background='Aquamarine')
    unblck_wn.title("Block a website")
    unblck_wn.geometry('400x300')
    unblck_wn.resizable(False, False)

    Label(unblck_wn, text='Unblock website', background='Aquamarine', font=("Georgia", 16)).place(x=120, y=20)
    Label(unblck_wn, text='Select the URL that you want to unblock:', background='Aquamarine', font=('Times',
                                                                                                      13)).place(x=60, y=90)
    ...
    dropdown.place(x=120, y=130)
    
    submit_btn = Button(unblck_wn, text='Submit', bg='Grey',
...
Label(root, text='Website Blocker', font=("Comic Sans MS", 16)).place(x=110, y=0)
